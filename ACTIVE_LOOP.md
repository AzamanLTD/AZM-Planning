...
- Backend main: `66b39b1` (2026-09-14); additionally the financial-integrity wave landed: PR #243 (`fdcd596`) atomic escrow dispute raise/resolve with canonical transaction-scoped financial primitives, and PR #244 (`66b39b1`) atomic business no-show settlement (canonical `_refundEscrowTx` refund + guarded stake penalty claim + conditional booking terminal transition + canonical audit in ONE transaction; guarded bookingEscrowService bucket drains; additive `BusinessProfile.stakeBalance` column, migration `20260914120000_business_stake_balance`). Exact-head CI green at `8c6e24e` (1181 tests incl. 10 real-PostgreSQL no-show proofs); post-merge main-head green at `66b39b1`. invoice creation/payment concurrency proofs remain verified with green exact-head tests and database recovery (PR #233 evidence unchanged). Since then: route-checker mount-registry fix (`6b7a884`), PR #236 P0 adapter recovery proof fixed and merged, the admin dine-in lifecycle projection endpoint (`117cebe`), and PR #241 control-plane integrity (squash `6d8da48`) — every privileged control-plane mutation now commits atomically with its STAFF audit event inside one transaction; lifecycle/PATCH updates are guarded against TOCTOU by the validated authority state (zero rows -> 409); and concurrent permission replacement is serialized with a row-level FOR UPDATE lock after the new concurrency test proved two parallel replacements committed a union of both grant sets. Exact-head CI green at `f5aabaa` (1149 tests incl. 11 real-PostgreSQL integrity tests), post-merge main-head run green at `6d8da48`; prod healthy. Schema fact recorded: control-plane tables exist only in migration `20260828040000_control_plane_staff_access` (not schema.prisma models), so `prisma db push` cannot provision them — the integrity suite applies that migration's DDL idempotently for prod-schema parity.
